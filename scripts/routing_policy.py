...
def _contract_shape(contract: Any) -> List[str]:
...
def _validate_role_file(path: Optional[Path], expected: Mapping[str, str]) -> bool:
    if path is None or tomllib is None:
        return False
    try:
        with path.open("rb") as handle:
    ...
    if "exactly these fields" not in instructions.lower() or any(f"`{field}`" not in instructions for field in EVIDENCE_FIELDS):
        return False
    if expected["role"] == "luna_max_fast" and (
        ...
        return False
    return all(data.get(key) == value for key, value in {
        "name": expected["role"],
        "model": expected["model"],
        "model_reasoning_effort": expected["reasoning"],
        "service_tier": expected["service_tier"],
        "sandbox_mode": expected["sandbox_mode"],
    }.items()) and isinstance(data.get("description"), str) and bool(data["description"].strip())
...
def verify_contract(root: Path | str = DEFAULT_ROOT) -> Dict[str, Any]:
    ...
    if root_path.is_symlink() or not root_path.is_dir():
        report["errors"] = ["runtime_root_unreadable"]
        return report
    policy_path = _safe_path(root_path, POLICY_RELATIVE)
    contract, parsed = _read_json(policy_path)
    if not parsed:
        report["errors"] = ["policy_unreadable"]
        return report
    errors = _contract_shape(contract)
    if errors:
...
def verify_active_root(
    ...
    repository_report = verify_contract(repository_root)
    ...
    if active_path.is_symlink() or not active_path.is_dir():
        report["errors"] = ["active_root_unreadable"]
        return report
    contract, parsed = _read_json(_safe_path(repository_path, POLICY_RELATIVE))
...
def evaluate(request: Any, root: Path | str = DEFAULT_ROOT) -> Dict[str, Any]:
    ...
    contract_report = verify_contract(root)
    ...
    if requested_role == "luna_max_fast":
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if command == "active-root":
        if not args.active_root:
            ...
            report = verify_active_root(args.active_root, args.root, args.active_config)
        print(_json_output(report) if args.format == "json" else _json_output(report))
        return 0 if report["ok"] else 1
    try:
