...
1. Confirm this is macOS and that git, Python 3.11 or newer, and codex are
   available. Show only their versions.
2. If $HOME/Developer/codex-sol-luna-orchestration-kit does not exist, create
   ...
   origin and stop for my direction if it is not the expected repository.
...
4. Run these static checks and stop on the first failure:
   PYTHONDONTWRITEBYTECODE=1 python3 scripts/verify_control_bundle.py --bundle control-bundles/all-max-v1 --format json
   PYTHONDONTWRITEBYTECODE=1 python3 scripts/routing_policy.py verify --root . --format json
...
5. Use exactly this dedicated pilot container:
   $HOME/codex-sol-luna-pilots/m4-v0.2.1-window-01
   Run the new-Mac preflight once with --dry-run and then with --apply:
   PYTHONDONTWRITEBYTECODE=1 python3 scripts/new_mac_preflight.py --pilot-home "$HOME/codex-sol-luna-pilots/m4-v0.2.1-window-01" --dry-run
   PYTHONDONTWRITEBYTECODE=1 python3 scripts/new_mac_preflight.py --pilot-home "$HOME/codex-sol-luna-pilots/m4-v0.2.1-window-01" --apply
...
6. Show me these two commands and pause while I complete each browser login
   ...
   CODEX_HOME="$HOME/codex-sol-luna-pilots/m4-v0.2.1-window-01/control/.codex" codex login
   CODEX_HOME="$HOME/codex-sol-luna-pilots/m4-v0.2.1-window-01/dynamic/.codex" codex login
...
7. Before any live model call, ask me to approve two small quota-consuming,
   ...
   control smoke first, then the dynamic smoke. For each root, use codex exec
   --ephemeral --skip-git-repo-check --sandbox read-only and
   --output-last-message. The prompt must say:
   "This is an unmeasured installation smoke, not pilot evidence. Spawn exactly
   one luna_scout_fast read-only subagent. Have it read no files, calculate
   6 times 7, and return only the integer. The runtime launch itself is the
   acceptance check and justifies delegation overhead. Wait for it. Reply
   exactly: ARM-smoke-ok:42"
   Replace ARM with control or dynamic. Verify the output file is exactly the
   expected line. Do not reuse or resume either session and do not run the two
   arms concurrently.
8. Re-run the environment and pilot status checks:
   PYTHONDONTWRITEBYTECODE=1 python3 scripts/pilot_tool.py verify-environments --pilot-home "$HOME/codex-sol-luna-pilots/m4-v0.2.1-window-01"
   PYTHONDONTWRITEBYTECODE=1 python3 scripts/pilot_tool.py status --pilot-home "$HOME/codex-sol-luna-pilots/m4-v0.2.1-window-01"
